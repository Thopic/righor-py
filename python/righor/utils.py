...
# def load_repseqio_vgenes(model, json_filename):
#     """ Given a model and a  repseqio json file (as produced by mixcr "findAlleles") 
#     replace the V and J genes of the model by the one generated by repseqio. 
#     Doesn't change the insertion/deletion profiles
#     """
#     with open(json_filename, 'r') as f:
#         genes = json.load(f)[0]['genes']

#     for g in genes:
#         name = g['name']
#         gene_type = g['geneType']
#         chains = g['chains']
#         functionality = 'True' if g['isFunctional'] else 'False'

#         if 'alleleInfo' in g:
#             ## load the other gene and apply the modifications


#         else:
#             database = g['baseSequence']
#             full_seq = efetch(database.split("://")[0], database.split("://")[1])
#             anchors = g['anchor_points']

#         if gene_type == 'V':
#             fr1_begin = anchors['FR1Begin']
#             vend = anchors['VEnd']
#             start, end = min(vend, fr1_begin), max(vend, fr1_begin)
#         if gene_type == 'J':
#             jend = anchors['FR4End']
#             jbegin = anchors['JBegin']
#             start, end = min(jend, jbegin), max(jend, jbegin)
            
#         gene_sequence = full_seq[start:end]
